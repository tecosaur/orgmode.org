...
*** New export engine and new exporters in contrib/

Nicolas new export engine org-export.el now lives in the =contrib/=
directory.  To use it:
...
If we have enough testers, the new export engine is very likely to 
replace the current one for the next major release.  Please report
...
org-element is a toolbox for parsing and analysing "elements" in 
an Org-mode buffer.  This has been written by Nicolas Goaziou and 
has been tested for quite some time.  It is now part of Org's core 
and many core functions rely on this package.
...
**** =org-up/down-element= (=C-c C-^= and =C-c C-_=)

This will move the point up/down in the hierarchy of elements.

**** =org-backward/forward-element= (=M-{= and =M-{=)
...
**** New option =org-src-prevent-auto-filling=

When non-nil, prevent auto-filling in source blocks.
...
**** org-feed now expand %(sexp) templates
...
*** M-TAB on options keywords (like #+TITLE) completes again
