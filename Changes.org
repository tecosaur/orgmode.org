...
This allows to compare times using seconds, not days, when honoring options
like =org-agenda-todo-ignore-*= in the agenda display.
...
org.el (org-deadline, org-schedule): Set the warning/delay relatively to
the timestamp
