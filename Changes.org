...
is valid for an entire column or for a rectangular section in a table."

*** Better handling of lists

Nicolas improved the handlings of lists.  FIXME
...
When set to a formatting string, insert this string.  See the docstring of
these variable for details about available %-sequences.

You can set =:html-preamble= in publishing project in the same way: `t'
means to honor =:email/creator/author-info=, and a formatting string will
insert a string.

*** New command `org-agenda-append-agenda'

You can now use `org-agenda-append-agenda' to dynamically add new agendas
views to the current one.  It is particularily useful to compare multiple
small agendas.

*** Localized clock tables

Clock tables now support a new :lang parameter, allowing the user to set
the language of the table headers.  =org-clock-clocktable-language-setup=
controls available translations.

*** New sorting options when publishing projects

The :sitemap-sort-file option now allows to sort (anti-)alphabetically and
(anti-)chronogically.  Thanks to Manuel Giraud for a patch to this effet.

*** Improvement of org-link-escape

David Maus worked on `org-link-escape'.
...
Org is now distributed with org-notmuch.el, by Matthieu Lemerre.

*** org-gnus.el now allows link creation from messages

You can now create links from messages.  This is particularily useful when
the user wants to stored messages that he sends, for later check.  Thanks
to Ulf Stegemann for the patch.

** Important bug fixes

*** Capturing to narrowed buffers

You can now safely capture entries to narrowed buffers.

*** Better handling of the new `org-agenda-span' variable

Agendas were a bit confused by the introduction of this variable, in
particular block agendas.  This is now fixed.

*** Stay clocked-in when capturing with (:immediate-finish t :clock-in t)

A capture template with =:immediate-finish t :clock-in t= used to clock
out.  You can now use both options safely.

*** Security warning: using org-crypt with auto-save

To prevent Emacs from auto-saving encrypted entries in clear text, the user
should not use auto-save with org-crypt.el.  We now send a warning when
users are both using auto-saving and org-crypt.el.
...
M-x org-agenda-bulk-mark-regexp RET will mark agenda entries which heading
match against a regular expression.  You can call this command with the `%'
from an agenda buffer.
...
Thanks to Matt Lundin for implementing this new functionality, and to Paul
Sexton for the idea.
...
When set to `nil', don't remove empty tables when exporting tables.
...
Effort duration can now be set as 2h (for 2 hours).  This will be converted
to minutes automatically when clocking in an entry with an effort property.

Thanks to Lawrence Mitchell for this patch.

*** LaTeX export

**** = emphasis now uses \protectedtexttt

**** You can now include author's email in the LaTeX title

When `org-export-email-info' is non-nil, the LaTeX title will also include
the author's email.
...
`org-export-latex-minted-options' allow package options to be controlled;
`org-export-latex-custom-lang-environments' allows arbitrary configuration
