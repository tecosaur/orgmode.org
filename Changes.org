...
*** Allow some commands to loop over siblings

=org-loop-over-siblings-within-active-region-p= is a new variable
that controls whether to use this new feature or not.  When set
to =non-nil=, the user can perform actions on several headlines
within the current active region.
...
For example, typing =C-c C-s= an a region with several headlines
will prompt for a =SCHEDULED= date for each entry successively.
...
Note that the variable =org-export-html-content-div= will still
be checked for compatibility reasons but is not a custom variable
anymore.
...
*** Export: new variable =org-export-html-before-content-div=

HTML code in this variable will be inserted /before/ the content
div.

[[http://orgmode.org/w/?p%3Dorg-mode.git%3Ba%3Dcommit%3Bh%3Dd20aa46a49370dd3eec583b26d6dd79bb4eb9ab2][(git commit)]]
...
*** Duration calculation now work for complex ranges in tables

Thanks to Christian Moe for signaling this problem.

** Details

*** New default value for =org-export-latex-image-default-option=
