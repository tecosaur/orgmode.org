...
such as [[https://jupyter.org/][Jupyter]] or
...
make it possible to leverage the Jupyter kernel ecosystem for even
more languages.
...
# Obscurable Outline
# Traversable trees
# Efficient information hiding
# Hideable Hierarchies
...
The fundamental structure of an Org document is a tree, with
collapsible sections forming the backbone of an Org document.  You can
...
Controlling section visibility only a tiny part of Org's complete set
of tools and commands for structured editing of plain text.  By
leveraging Org's regular syntax to transform otherwise tedious tasks
into single key commands, Org mode frees you to focus on content
rather than form.
...
through task states with the shortcuts =S-<left>= and =S-<right>=.
...
Everything in Org mode is visible. Nothing is hidden.
This is true for tables as well. If you care about
data provenance, look no further.
...
Org is an authoring and publishing tool.  Org can be everything from a
static site generator, to a suite of tools for self-publishing.
...
** Description
Org mode isn't just capable out of the box, and it isn't just
extensible, it is /Emacs extensible/.

To give only a single example, consider hyperlinks.  While updating
docs for a project you find yourself repeatedly linking to GitHub
issues. In Org you can add a new ~issue:~ link type to the document
(or globally if you prefer) so that ~[[issue:74][version mismatch]]~
expands to =https://github.com/author/repo/issues/74=.
...
With /agenda views/ --- reports generated from a set of tasks --- you
...
The default agenda shows scheduled and deadlined tasks for the
week, stalled projects, and all your outstanding TODO items.
...
working in an =.org= file =C-c C-x C-i= clocks in, and =C-c C-x C-o=
clocks out. Time is stored in the property drawer of the headline for
the task at had.

You can also use =I= and =O= when in an agenda view.

At end of the day, you can easily generate customized reports about
...
On iOS you have the wonderful [[https://beorgapp.com/][beorg]], which
supports many of Org's features and can sync over a variety of
services.

On Android you have the excellent and open source [[http://www.orgzly.com/][orgzly]],
which likewise supports many of Org's features and can sync over a variety of services.
([[https://github.com/orgzly/orgzly-android][Git repo]]).

In addition to native apps [[https://github.com/200ok-ch/organice][organice]]
implements Org mode as a web app that supports mobile and desktop browsers.
You can host the app yourself or access a version hosted by the developer at
https://organice.200ok.ch/.

There is also [[https://github.com/DanielDe/org-web][org-web]] which takes a
similar approach, and that can be self-hosted or accessed via https://org-web.org/.
